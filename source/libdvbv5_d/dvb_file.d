 ...
 * @ingroup file
 */
extern __gshared const dvb_parse_file channel_file_format;

/**
 * @brief File format definitions for dvb-apps zap format
 * @ingroup file
 */
extern __gshared const dvb_parse_file channel_file_zap_format;
 ...
 * were read from the file. If it fails, NULL is returned.
 */
dvb_file* dvb_read_file_format (
    const(char)* fname,
    uint delsys,
    dvb_file_formats format);
 ...
 */
int dvb_store_channel (
    dvb_file** dvb_file,
    dvb_v5_fe_parms* parms,
    dvb_v5_descriptors* dvb_desc,
    int get_detected,
    int get_nit);
 ...
 */
dvb_file* dvb_parse_format_oneline (
    const(char)* fname,
    uint delsys,
    const(dvb_parse_file)* parse_file);
