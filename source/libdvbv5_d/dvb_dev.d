 ...
 */
alias dvb_dev_change_t = int function (
    char* sysname,
    dvb_dev_change_type type,
    void* priv);
...
struct dvb_device
{
    ...
    dvb_dev_list* devices;
    int num_devices;

    /* Digital TV frontend access */
    dvb_v5_fe_parms* fe_parms;
}
 ...
 */
dvb_dev_list* dvb_dev_seek_by_adapter (
    dvb_device* dvb,
    uint adapter,
    uint num,
    dvb_dev_type type);
 ...
 */
dvb_open_descriptor* dvb_dev_open (
    dvb_device* dvb,
    const(char)* sysname,
    int flags);
 ...
 * @note valid only for DVB_DEVICE_DEMUX.
 */
int dvb_dev_dmx_set_pesfilter (
    dvb_open_descriptor* open_dev,
    int pid,
    dmx_pes_type_t type,
    dmx_output_t output,
    int buffersize);
 ...
 * @note valid only for DVB_DEVICE_DEMUX.
 */
int dvb_dev_dmx_set_section_filter (
    dvb_open_descriptor* open_dev,
    int pid,
    uint filtsize,
    ubyte* filter,
 ...
 */
dvb_v5_descriptors* dvb_dev_scan (
    dvb_open_descriptor* open_dev,
    dvb_entry* entry,
